...
void init_active() {
	...
	int64_t cid=get_container_count();
	archive_container = cid>1 ? retrieve_archive_container_by_id(cid-1) : NULL;
...
void close_active() {
	...
	TIMER_END(1, remove_time);
	remove_time = remove_time / 1000000;
	//remove_time = (double)(clock() - start) / CLOCKS_PER_SEC;
...
void active_remove(struct doubleBuffer *t){
	...
	while(g_hash_table_iter_next(&iter, &key, &value)){
		...
		if (destor.simulation_level >= SIMULATION_RESTORE) {
			...
			SHA_CTX ctx;
			SHA1_Init(&ctx);
			SHA1_Update(&ctx, ck->data, ck->size);
			SHA1_Final(ck->fp, &ctx);
