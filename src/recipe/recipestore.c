...
void init_recipe_store() {
	...
	//NOTICE("init_recipe_sotre: read from %s", count_fname);
	sdsfree(count_fname);

    NOTICE("Init recipe store successfully, backupversion: %d", backup_version_count);
}
...
void free_segment_recipe(void* sr) {
	g_hash_table_destroy( ((struct segmentRecipe*)sr)->kvpairs);
	free(sr);
}
...
int segment_recipe_check_id(void* sr, void *id) {
     return ((struct segmentRecipe*)sr)->id == *((segmentid *)id);
}
...
int lookup_fingerprint_in_segment_recipe(void* sr,
        void *fp) {
    return g_hash_table_lookup( ((struct segmentRecipe*)sr)->kvpairs, (fingerprint *)fp) == NULL ? 0 : 1;
}
